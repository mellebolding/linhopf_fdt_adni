...
# ==================== DATA SETUP ====================
# filename = f"FDT_results_DL_B2_N400_sigTrue_aTrue.npz"
...
def nestedCV(Xdict, Y, varnames):
    ...
    UPDATES:
    - Can now accept specific feature names (e.g. ['I_norm2_1', 'Tau_5']) in 'varnames'.
    - Automatically detects if input is modality keys or specific features.
    """
    
    # =========================================================================
    # LOGIC TO HANDLE SPECIFIC FEATURE NAMES VS MODALITY KEYS
    # =========================================================================
    
    # Check if the first element of varnames is a direct key in Xdict
    # If yes, assume we are using the standard "Whole Modality" mode
    ...
    # If no, assume we are providing a specific list of feature names (FN)
    else:
        print(f"Input detected as Specific Feature List ({len(varnames)} features).")

        
        # 1. Infer which modalities are needed based on the feature prefixes
        ...
        # 2. Load the full universe of features for these modalities
        ...
        print(f"\n--- DEBUGGING FEATURE NAME MISMATCH ---")
        print(f"1. Name requested by you (example): '{varnames[0]}'")
        print(f"2. Name generated by code (example): '{names_full[0]}'")
        print(f"3. Total generated names: {len(names_full)}")
        
        # Check if the requested name exists in the generated list
        if varnames[0] in names_full:
            print("   -> Match found! (The problem might be specific missing features)")
        else:
            print("   -> NO MATCH. The naming convention differs.")
            print("      Likely cause: 'regions' variable is different or 'extendFeatureNames' behaves differently.")
        print("-" * 40 + "\n")


        # 3. Create a map for fast index lookup
        name_to_idx = {name: i for i, name in enumerate(names_full)}
        
        # 4. Find indices of the requested features
        indices_to_keep = []
        valid_feature_names = []
    ...
    # Use StratifiedShuffleSplit (as per your previous code)
    ...
    for train_outer, test_outer in cv_outer.split(X, Y):
        ...
        if len(fidx) > 0:
            ...
            # Fallback if no features were selected (unlikely with this logic, but good practice)
            score = 0
            yhat = np.full_like(Ytest, fill_value=Ytrain[0]) # Dummy prediction
...
# print("="*60)
    ...
    flat_features1 = flat_features1[:100]
    flat_features2 = flat_features2[:100]
# print("flat_features1:", flat_features1)
# print("flat_features2:", flat_features2)

# 2. Combine and get unique feature names (remove duplicates)
# We use set() to handle uniqueness, then convert back to list
...
output_file = 'nested_cv_results.npz'
...
np.savez(
    ...
    YT1=YT1,
    TI1=TI1,
    feat_names1=feat_names1,  # NumPy handles lists of strings/objects via pickle
    sf2=sf2_list,
    FI2=FI2,
    FX2=FX2,
    FN2=FN2,
